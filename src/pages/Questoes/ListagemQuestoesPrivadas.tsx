	...
	useEffect(() => {
		async function carregarQuestoes() {
			...
			try {
				...
				console.error(error);
				setErro("Erro ao carregar questões privadas. Tente novamente.");
			} finally {
				setCarregando(false);
			}
		}
			...
			<div className="max-w-5xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
					...
					<div className="flex items-center gap-3 sm:gap-4">
						...
						<div>
							...
							<p className="text-gray-500 text-sm sm:text-base mt-0.5 sm:mt-1">
								Suas questões que não são visíveis para outros
							</p>
						</div>
					</div>
				</div>
				...
				{carregando ? (
					...
					<div className="text-center py-20 text-gray-400">
						...
						<p className="text-lg">Você não possui questões privadas ainda.</p>
						<p className="text-sm mt-1">
							Ao criar uma questão, escolha a opção "Privada" para que ela
							apareça aqui.
						</p>
					</div>
				) : (
